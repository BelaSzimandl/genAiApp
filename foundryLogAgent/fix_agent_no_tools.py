...
sample_csv = Path(r"c:\Users\Bela_Szimandl\Documents\Repos\genAiApp\stockAiApp\data\sample_logs.csv").read_text(encoding="utf-8")
...
# No FunctionTool list: custom function tools show "Not supported by the selected model"
# for gpt-5-mini in Foundry playground and cannot execute without a tool host.
...
print("updated", agent.name, "version", agent.version, "status", agent.status)
print("tools", getattr(agent.definition, "tools", None))

openai = client.get_openai_client()
resp = openai.responses.create(
    model="gpt-5-mini",
    input="What's the system health? Count ERROR vs total.",
